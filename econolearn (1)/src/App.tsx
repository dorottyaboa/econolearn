...
export default function App() {
  const [level, setLevel] = useState<Level>(() => (localStorage.getItem("econ_level") as Level) || "Beginner");
  const [topic, setTopic] = useState<string>(() => localStorage.getItem("econ_topic") || "Supply & Demand");
  const [page, setPage] = useState<Page>("Home");
  const [xp, setXp] = useState(() => Number(localStorage.getItem("econ_xp")) || 0);
  const [streak, setStreak] = useState(() => Number(localStorage.getItem("econ_streak")) || 0);
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem("econ_darkMode") === "true");
  const [totalAnswered, setTotalAnswered] = useState(() => Number(localStorage.getItem("econ_totalAnswered")) || 0);
  const [correctAnswered, setCorrectAnswered] = useState(() => Number(localStorage.getItem("econ_correctAnswered")) || 0);
  const [dailyDone, setDailyDone] = useState<string[]>(() => JSON.parse(localStorage.getItem("econ_dailyDone") || "[]"));
  const [quizAnswers, setQuizAnswers] = useState<Record<string, { chosen: string; correct: boolean }>>(() => JSON.parse(localStorage.getItem("econ_quizAnswers") || "{}"));
  const [scenarioRevealed, setScenarioRevealed] = useState<Record<string, boolean>>(() => JSON.parse(localStorage.getItem("econ_scenarioRevealed") || "{}"));
  const [fibAnswers, setFibAnswers] = useState<Record<string, string>>({});
  const [fibSubmitted, setFibSubmitted] = useState<Record<string, string>>(() => JSON.parse(localStorage.getItem("econ_fibSubmitted") || "{}"));
