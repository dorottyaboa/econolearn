...
def sync_to_firestore():
    if st.session_state.user:
        user_ref = doc(db, "users", st.session_state.user['uid'])
        try:
            setDoc(user_ref, {
                ...
                "completedNodes": st.session_state.completedNodes,
                "lastUpdated": serverTimestamp()
            }, merge=True)
        except Exception as e:
            handleFirestoreError(e, OperationType.WRITE, f"users/{st.session_state.user['uid']}")
...
def check_badges():
    ...
    # Sync after checking badges or any progress update
    sync_to_firestore()
    ...
    with cols[0]:
        if st.session_state.user:
            ...
            if st.button("Login", key="login_btn"):
                try:
                    # Note: In a real streamlit app, this might need a custom component 
                    # for popup auth, but we'll use a placeholder for now as 
                    # direct JS interop is limited in standard streamlit.
                    # We assume the user is logged in for the demo if they click.
